# -*- coding: utf-8 -*-
"""Tests for Bokeh PlotResPhasePseudoSection MTData compatibility."""
...
def bokeh_plot_pseudosection_class():
    """Import Bokeh PlotResPhasePseudoSection or skip if Bokeh is unavailable."""

    pytest.importorskip("bokeh")
    from mtpy.imaging.bokeh_plots import PlotResPhasePseudoSection

    return PlotResPhasePseudoSection
...
class TestPlotResPhasePseudoSectionBokehMTData:
    def test_iter_mt_objects_from_tree(
        self, bokeh_plot_pseudosection_class, mt_data_tree
    ):
    ...
    def test_get_data_df_from_tree(
        self, bokeh_plot_pseudosection_class, mt_data_tree, monkeypatch
    ):
        plotter = bokeh_plot_pseudosection_class(mt_data_tree, show_plot=False)
    ...
    def test_get_profile_line_syncs_profile_offsets_to_tree(
        self, bokeh_plot_pseudosection_class, mt_data_tree
    ):
        plotter = bokeh_plot_pseudosection_class(mt_data_tree, show_plot=False)
    ...
    def test_get_offset_prefers_station_locations_profile_offset(
        self, bokeh_plot_pseudosection_class, mt_data_tree
    ):
        plotter = bokeh_plot_pseudosection_class(mt_data_tree, show_plot=False)
    ...
    def test_plot_runs_with_tree_backed_data(
        ...
        monkeypatch.setattr(
            "mtpy.imaging.bokeh_plots.plot_pseudosection.griddata_interpolate",
    ...
    def test_constructor_honors_station_id_alias(
        self, bokeh_plot_pseudosection_class, mt_data_tree
    ):
        plotter = bokeh_plot_pseudosection_class(
            mt_data_tree,
            show_plot=False,
            stationid=[1, None],
        )

        assert plotter.station_id == [1, None]
        assert plotter.stationid == [1, None]
