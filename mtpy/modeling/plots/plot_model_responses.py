...
class PlotResponse:
    """Plot ModEM data and model responses.

    ModEM data files are loaded into :class:`mtpy.MTData` using the ``data``
    survey; model response files use the ``model`` survey.  This class keeps
    the existing plot styles while reading stations through the tree-backed
    MTData API.
    ...
    def __init__(
        self,
        data_fn: str | Path | None = None,
        resp_fn: str | Path | list[str | Path] | None = None,
        **kwargs: Any,
    ) -> None:
        """Initialize a ModEM response plot.

        Parameters
        ----------
        data_fn : str or pathlib.Path, optional
            ModEM observed-data file.
        resp_fn : str, pathlib.Path, or list, optional
            One or more ModEM model-response files.
        **kwargs
            Plot appearance and layout options.
        """
        self.data_fn = data_fn
        self.resp_fn = resp_fn

        self.data_object: MTData | None = None
        ...
        self.e_capthick = kwargs.pop("capthick", 0.5)
        self.e_capsize = kwargs.pop("capsize", 2)
    ...
    def plot(self) -> None:
        """Create figures using the configured plot style."""
    ...
    def _read_files(self) -> None:
        """Read observed data and model responses into MTData containers.

        Raises
        ------
        ValueError
            If no observed ModEM data filename was supplied.
        """
        if self.data_fn is None:
            raise ValueError("data_fn is required to plot a ModEM response")

        self.data_object = MTData()
        self.data_object.from_modem(self.data_fn, survey="data")

        response_files = (
            self.resp_fn if isinstance(self.resp_fn, list) else [self.resp_fn]
        )
        for count, response_file in enumerate(response_files, 1):
            if response_file is None:
                continue
            self.data_object.from_modem(response_file, survey=f"model_{count:02}")

    def _station_names(self, mt_data: MTData) -> list[str]:
        """Return station names in the order stored by an MTData container."""
        return [
            station_path.rsplit("/", 1)[-1]
            for station_path in mt_data._iter_station_paths()
        ]

    def _get_station_list(self, station: str) -> list[str]:
        "get a list of stations that have the same station stem"
        return [
            station_path
            for station_path in self.data_object._iter_station_paths()
            if station_path.rsplit("/", 1)[-1] == station
        ]

    def _get_data_station(self, station: str) -> MT:
        """Return a station MT object from an MTData survey.

        Parameters
        ----------
        mt_data : MTData
            Tree-backed ModEM data container.
        station : str
            Station name to retrieve.

        Returns
        -------
        mtpy.MT
            Reconstructed transfer-function object for *station*.
        """
        station_path = f"data/{station}"
        return self.data_object.get_station(station_path, as_mt=True)

    def _get_response_station(self, station: str) -> list[MT]:
        """get a list of responses"""
        resp_list = []
        for resp_path in self._get_station_list(station):
            if "model" in resp_path:
                resp_list.append(self.data_object.get_station(resp_path, as_mt=True))
        return resp_list

    def _plot(self) -> None:
        """Plot individual impedance tensor components in four columns."""

        # --> set default font size
        plt.rcParams["font.size"] = self.font_size

        fontdict = {"fontsize": self.font_size + 2, "fontweight": "bold"}
        if self.plot_z:
            h_ratio = [1, 1, 0.5]
        ...
        mt_obj = self._get_data_station(self.plot_station)
        z_obj = mt_obj.Z
        t_obj = mt_obj.Tipper
        if t_obj is None:
            self.plot_tipper = False
        elif np.all(t_obj.tipper == 0.0) == True:
            self.plot_tipper = False
        period = z_obj.period
        print(f"Plotting: {self.plot_station}")
        ...
        if self.plot_z:
            ...
            plot_res_err = abs(z_obj.z_model_error * scaling)
            plot_phase = abs(z_obj.z.imag * scaling)
            plot_phase_err = abs(z_obj.z_model_error * scaling)
            h_ratio = [1, 1, 0.5]

        elif not self.plot_z:
            plot_res = z_obj.resistivity
            plot_res_err = z_obj.resistivity_model_error
            plot_phase = z_obj.phase
            plot_phase_err = z_obj.phase_model_error
            h_ratio = [1.5, 1, 0.5]
            ...
            try:
                    ...
                    ** (
                        np.floor(
                            np.log10(
                                np.nanmin(
                                    [
                                        np.nanmin(plot_res[nzxx, 0, 0]),
                                        np.nanmin(plot_res[nzyy, 1, 1]),
                                    ]
                                )
                            )
                        )
                    ...
                    ** (
                        np.ceil(
                            np.log10(
                                np.nanmax(
                                    [
                                        np.nanmax(plot_res[nzxx, 0, 0]),
                                        np.nanmax(plot_res[nzyy, 1, 1]),
                                    ]
                                )
                            )
                        )
                ...
                if np.nan in self.res_limits_d:
                    ...
                    ** (
                        np.floor(
                            np.log10(
                                np.nanmin(
                                    [
                                        np.nanmin(plot_res[nzxy, 0, 1]),
                                        np.nanmin(plot_res[nzyx, 1, 0]),
                                    ]
                                )
                            )
                        )
                    ...
                    ** (
                        np.ceil(
                            np.log10(
                                np.nanmax(
                                    [
                                        np.nanmax(plot_res[nzxy, 0, 1]),
                                        np.nanmax(plot_res[nzyx, 1, 0]),
                                    ]
                                )
                            )
                        )
            ...
            # make figure
            fig = plt.figure(self.plot_station, self.fig_size, dpi=self.fig_dpi)
            plt.clf()
            fig.suptitle(str(self.plot_station), fontdict=fontdict)
            ...
            if not self.plot_tipper:
                ...
                self.tipper_limits = (
                    np.round(
                        np.nanmin(
                            [
                                np.nanmin(t_obj.tipper[ntx, 0, 0].real),
                                np.nanmin(t_obj.tipper[nty, 0, 1].real),
                                np.nanmin(t_obj.tipper[ntx, 0, 0].imag),
                                np.nanmin(t_obj.tipper[nty, 0, 1].imag),
                            ]
                        ),
                        1,
                    ),
                    np.round(
                        np.nanmax(
                            [
                                np.nanmax(t_obj.tipper[ntx, 0, 0].real),
                                np.nanmax(t_obj.tipper[nty, 0, 1].real),
                                np.nanmax(t_obj.tipper[ntx, 0, 0].imag),
                                np.nanmax(t_obj.tipper[nty, 0, 1].imag),
                            ]
                        ),
                        1,
                    ),
            ...
            print((f"self.plot_tipper = {self.plot_tipper}"))
            # ----------------------------------------------
            ...
            if not self.plot_tipper:
                try:
                    ...
                    print(f"Found no Z components for {self.plot_station}")
            ...
            # plot model response
            resp_list = self._get_response_station(self.plot_station)
            if resp_list not in [None, []]:
            ...
            if self.save_plots:
                save_filename = Path(self.data_fn).parent.joinpath(
                    f"{self.plot_station}.png"
                )
                self.save_figure(save_filename, fig_dpi=self.fig_dpi)

    def _plot_2col(self) -> None:
        """Plot observed and model responses in two columns."""
        ...
        if plot_tipper == True:
            ...
            if self.plot_style == 2:
                #                z_obj.phase_xy[z_obj.phase_xy > 180] -= 360
                #                z_obj.phase_yx[z_obj.phase_yx > 180] -= 360
                if self.plot_component == 2:
                    ...
                    if self.plot_z == False:
                        ...
                        erxy = plot_errorbar(
                            ...
                            period[nzxy],
                            z_obj.phase_xy[nzxy],
                            z_obj.phase_model_error_xy[nzxy],
                            **kw_xx,
                        )
                        eryx = plot_errorbar(
                            axpxy,
                            period[nzyx],
                            z_obj.phase_yx[nzyx],
                            z_obj.phase_model_error_yx[nzyx],
                            **kw_yy,
                        )
                    elif self.plot_z == True:
                        # plot real
                        erxy = plot_errorbar(
                            ...
                            abs(z_obj.z_model_error[nzxy, 0, 1].real),
                            **kw_xx,
                        )
                        eryx = plot_errorbar(
                            axrxy,
                            ...
                            abs(z_obj.z_model_error[nzxy, 1, 0].real),
                            **kw_yy,
                        )
                        # plot phase
                        erxy = plot_errorbar(
                        ...
                        eryx = plot_errorbar(
                            ...
                            abs(z_obj.z_model_error[nzyx, 1, 0].real),
                            **kw_yy,
                        )
                    # plot tipper
                    if plot_tipper == True:
                    ...
                    if self.plot_z == False:
                        ...
                        erxx = plot_errorbar(
                            ...
                            period[nzxx],
                            z_obj.phase_xx[nzxx],
                            z_obj.phase_model_error_xx[nzxx],
                            **kw_xx,
                        )
                        erxy = plot_errorbar(
                            axpxy,
                        ...
                        eryy = plot_errorbar(
                            ...
                            period[nzyy],
                            z_obj.phase_yy[nzyy],
                            z_obj.phase_model_error_yy[nzyy],
                            **kw_yy,
                        )
                    elif self.plot_z == True:
                        # plot real
                        erxx = plot_errorbar(
                            ...
                            abs(z_obj.z_model_error[nzxx, 0, 0].real),
                            **kw_xx,
                        )
                        erxy = plot_errorbar(
                            axrxy,
                        ...
                        eryy = plot_errorbar(
                            ...
                            abs(z_obj.z_model_error[nzyy, 1, 1].real),
                            **kw_yy,
                        )
                        # plot phase
                        erxx = plot_errorbar(
            ...
            if plotr == True:
                ...
                for rr in range(nr):
                    ...
                    response_mt = resp_list[rr]
                    resp_z_obj = response_mt.Z
                    resp_z_err = np.nan_to_num(
                        (z_obj.z - resp_z_obj.z) / z_obj.z_model_error
                    )

                    resp_t_obj = response_mt.Tipper
                    resp_t_err = np.nan_to_num(
                        (t_obj.tipper - resp_t_obj.tipper) / t_obj.tipper_model_error
                    )
                    ...
                    if self.plot_style == 2:
                        #                        resp_z_obj.phase_xy[resp_z_obj.phase_xy > 180] -= 360
                        #                        resp_z_obj.phase_yx[resp_z_obj.phase_yx > 180] -= 360
                        ...
                        if self.plot_component == 2:
                            if self.plot_z == False:
                                ...
                                rerxy = plot_errorbar(
                                    axrxy,
                                    period[nzxy],
                                    resp_z_obj.res_xy[nzxy],
                                    **kw_xx,
                                )
                                reryx = plot_errorbar(
                                    axrxy,
                                    period[nzyx],
                                    resp_z_obj.res_yx[nzyx],
                                    **kw_yy,
                                )
                                # plot phase
                                rerxy = plot_errorbar(
                                    axpxy,
                                    period[nzxy],
                                    resp_z_obj.phase_xy[nzxy],
                                    **kw_xx,
                                )
                                reryx = plot_errorbar(
                                    axpxy,
                                    period[nzyx],
                                    resp_z_obj.phase_yx[nzyx],
                                    **kw_yy,
                                )
                            elif self.plot_z == True:
                                # plot real
    ...
    def _get_station_index_list(self) -> range | list[int]:
        """Return indices for the requested observed-data stations.

        Returns
        -------
        range or list of int
            Indices into the observed-data station ordering.
        """

        allstations = self._station_names(self.data_object)
    ...
    def _plot_1col(self) -> None:
        """Plot all impedance components overlaid in one column."""
        # read files
        self._read_files()
        ...
        if self.period_limits is None:
            self.period_limits = (
                ...
                ** (
                    np.floor(
                        np.log10(
                            self._get_mt_station(
                                self.data_object,
                                self._station_names(self.data_object)[0],
                            ).Z.period[0]
                        )
                    )
                )
        ...
        station_names = self._station_names(self.data_object)
        for ii, si in enumerate(s_index_list):
            sname = station_names[si]
            ...
            for di in range(len(data_objects)):
                ...
                mt_obj = self._get_mt_station(dObj, sname)
                zObj = mt_obj.Z
                tObj = mt_obj.Tipper
                ...
                if self.plot_z:
                    ...
                    data1label = "Z (real)"
                    data2label = "Z (imag)"
                    data1err = zObj.z_model_error
                    data2err = zObj.z_model_error
                    axp.set_yscale("log")
                else:
                ...
                for i in range(2):
                    ...
                    tipper_model_error = tObj.tipper_model_error[nonzerot][:, 0, i]
                    if di >= 1:
                        tipper_model_error = np.zeros_like(tipper_model_error)
    ...
    def redraw_plot(self) -> None:
        """Redraw figures after updating plot configuration.

        Examples
        --------
        Update a style property and redraw:

        >>> response_plot.lw = 2
        >>> response_plot.redraw_plot()
        """
    ...
    def save_figure(
        ...
        """Save the active response plot.

        Parameters
        ----------
        save_fn : str or pathlib.Path
            Output filename or destination directory.
        file_format : str, optional
            Output format when *save_fn* is a directory, by default ``"pdf"``.
        orientation : str, optional
            Figure orientation, by default ``"portrait"``.
        fig_dpi : int, optional
            Output resolution. Uses the figure DPI when omitted.
        close_fig : {True, False}, optional
            Whether to close the active figure after saving, by default ``True``.
        """
        ...
        if save_fn.is_dir() == False:
            # file_format = save_fn.suffix.replace(".", "")
            fig.savefig(
                save_fn,
                dpi=fig_dpi,
                format=file_format,
            ...
            save_fn = save_fn / f"{self.station}_L2.{file_format}"
            fig.savefig(
                save_fn.as_posix(),
                dpi=fig_dpi,
                format=file_format,
